...
func TestRecordPostgresPoolStats_NilSafe(t *testing.T) {
	require.NoError(t, RecordPostgresPoolStats(nil))
}

func TestRecordPostgresPoolStats_FromPool(t *testing.T) {
	cfg, err := pgxpool.ParseConfig("postgres://user:pass@127.0.0.1:1/db?sslmode=disable")
	require.NoError(t, err)
	cfg.MaxConns = 7
	...
	// Records max_open and other gauges from pgxpool.Stat without panicking.
	require.NoError(t, RecordPostgresPoolStats(pool))
	require.Equal(t, int32(7), pool.Stat().MaxConns())
}
